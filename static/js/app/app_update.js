...
$(document).ready(function(){
	$('#mibox1').uploadify({
		...
		'onUploadSuccess'	:function(file, data, response){
			...
			if(result.ret == 1){
				...
				$('#mibox1_checkbox').attr("checked", "checked");
	...
	$('#upload_package').uploadify({
		...
		'uploader'			:'http://172.27.9.104:80/uploadfile?type=apk&_xsrf='+$("input[name='_xsrf']").val()+"&user="+getCookie('user')+
		                     '&package_name_old='+$("input[name='package_name_old']").val()+'&version_code_old='+$("input[name='version_code_old']").val(),
		'formData'		 	:{'_xsrf':$("input[name='_xsrf']").val(), 'user':getCookie('user'), 
		                      'package_name_old':$("input[name='package_name_old']").val(),
		                      'version_code_old':$("input[name='version_code_old']").val()},
		...
		'onUploadSuccess': function(file, data, response){
			...
			if (result.ret != 1){
     		    if(result.flag == -2){
					alert("Package和现有名称不符，请选择正确的APK");		
					return;    
			    }else if(result.flag == -3){
			        alert("您上传的APK的版本号小于等于旧的APK,若需更新请上传新版APK");
			    }
			    return;
			}
			//失败直接返回，不要再去强行赋值
			$('#upload_status').text('Apk文件开始向CDN中上传，请稍候...');
             
			$('input[name="byte_size"]').val(result.byte_size);
			$('input[name="upload_time"]').val(result.upload_time);
			$('input[name="install_url"]').val(result.install_url);
